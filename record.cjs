...
console.log('🎬 720p@24fps→Lanczos 1080p');

(async () => {
  const browser = await chromium.launch({ headless: true, args: ['--no-sandbox','--disable-gpu','--disable-background-timer-throttling'] });
  const ctx = await browser.newContext({ viewport: { width: 1280, height: 720 }, recordVideo: { dir: 'd:/Desktop/AI相册', size: { width: 1280, height: 720 }, fps: 24 } });
  ...
  console.log('🔊 BGM + CRF16 + Lanczos 1080p...');
  execSync(`"${FFMPEG}" -i "${raw}" -i "d:/Desktop/AI相册/想你和我们的以后.mp3" -c:v libx264 -preset fast -crf 16 -vf "scale=1920:1080:flags=lanczos,fps=24" -c:a aac -b:a 192k -map 0:v:0 -map 1:a:0 -filter:a "volume=0.55" -map_metadata -1 -map_chapters -1 -t 47 -movflags +faststart -y "${OUT}"`, { stdio: 'inherit' });
